  ...
  */
  ...
  while ((key = (TKey*)next3())) {
      ...
      // read fijs for each error, and use the untrivial fijs. DO NOT constrain CC_nutau_xsec.
      {
        ...
        TRandom3* r3 = new TRandom3();
        r3->SetSeed(0);
        float error_val = r3->Gaus();
        if(error_val>1)
            error_val = 1;
        else if(error_val<-1)
            error_val = -1;
        //if(r3->Rndm()>0.9)tau_fit->var(error_term.c_str())->setVal(error_val);
  ...
  {
    RooRandom::randomGenerator()->SetSeed(0);
    TString root_file = "./mc_study/mc_tau_" ;
    root_file += job;
    root_file += ".root";
    TFile* mc_study = new TFile(root_file,"recreate");
    ...
    for(int i = 0; i <2; i++)// Create MC sample for study.
    {
        ...
        RooDataSet  *mc_sigI = modelI.generate(axisVariables, TMath::Nint(r3->PoissonD(1.015*bkgHisto2DZenithSKI->Integral()+1.0*tauHisto2DZenithSKI->Integral())));
        RooDataSet  *mc_sigII = modelII.generate(axisVariables, TMath::Nint(r3->PoissonD(1.015*bkgHisto2DZenithSKII->Integral()+1.0*tauHisto2DZenithSKII->Integral())));
        RooDataSet  *mc_sigIII = modelIII.generate(axisVariables, TMath::Nint(r3->PoissonD(1.015*bkgHisto2DZenithSKIII->Integral()+1.0*tauHisto2DZenithSKIII->Integral())));
        RooDataSet  *mc_sigIV = modelIV.generate(axisVariables, TMath::Nint(r3->PoissonD(1.015*bkgHisto2DZenithSKIV->Integral()+1.0*tauHisto2DZenithSKIV->Integral())));
        /*cc_nutau_xsec.setVal(1.0);
